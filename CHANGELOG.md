...
- **Kompletna naprawa diagramów Mermaid na urządzeniach fizycznych Android (`MarkdownText.kt`, `MarkdownTextTest.kt`)**:
  - **Rozwiązanie problemu limitu bufora transakcji IPC/Binder Androida (1 MB)**: W poprzedniej poprawce skrypt biblioteki Mermaid (3.2 MB) został bezpośrednio wstrzyknięty do łańcucha znaków `htmlContent`. Na urządzeniach mobilnych Android WebView renderowany jest w osobnym procesie piaskownicy (`sandboxed_process0`), a komunikacja międzyprocesowa (Binder IPC) posiada twardy limit bufora transakcji 1 MB. Przekroczenie tego limitu powodowało ciche odrzucenie załadowania strony przez silnik WebView, co objawiało się pustym ciemnoniebieskim kafelkiem i zniknięciem wskaźnika ładowania po upływie limitu czasu.
  - **Bezpieczne, bezpośrednie strumieniowanie assetu przez `WebViewClient`**: Przywrócono lekki szablon HTML (~2 KB), a ładowanie pliku `mermaid.min.js` (3.2 MB) zrealizowano za pomocą standardowego znacznika `<script src="https://appassets.androidplatform.net/assets/mermaid/mermaid.min.js">`.
  - **Natychmiastowa inicjalizacja `webViewClient` w `remember`**: Klient `WebViewClient` jest teraz konfigurowany natychmiast podczas tworzenia instancji `WebView` (a nie w opóźnionym `DisposableEffect`), a metoda `shouldInterceptRequest` bezpośrednio przechwytuje żądanie dla `mermaid.min.js` i strumieniuje plik z `context.assets.open(...)` z pominięciem ograniczeń wielkości pamięci Binder.
  - **Zwiększenie limitu czasu oczekiwania**: Zwiększono limit prób w pętli `renderDiagram()` z 60 (6s) do 150 (15s), dając słabszym urządzeniom wystarczająco dużo czasu na sparsowanie biblioteki.
- **Wzory matematyczne KaTeX – dolny margines i brak ucinania (`MarkdownText.kt`, `MarkdownTextTest.kt`)**:
  - **Zwiększenie dolnego odstępu w `#math-container`**: Zwiększono padding z `10px 48px 10px 18px` do `12px 48px 22px 18px`, co zapewnia symetryczną przestrzeń od dołu dla indeksów dolnych, ułamków, sum oraz całek.
  - **Ochrona paska przewijania w `#math-scroll`**: Dodano `padding-bottom: 6px`, dzięki czemu poziomy pasek przewijania nie nachodzi na dolne elementy wzoru.
  - **Zwiększenie bufora wysokości w mostku JS/Kotlin**: Zwiększono bufor raportowania wysokości z `h + 8` do `h + 16` oraz dołożono dodatkowe `8.dp` w Compose, dzięki czemu dolny margines karty matematycznej jest w pełni zgodny z górnym.
- **Prawidłowe pozycjonowanie okien modalnych (`FileViewerDialog.kt`, `PermissionsAuditDialog.kt`)**:
  - **Usunięcie podwójnego przesunięcia w dół i nakładania na dolną nawigację systemową**: Usunięto flagę `decorFitsSystemWindows = false` z `DialogProperties` oraz ręczne dodawanie `effectiveTopInset` (wysokość paska stanu ~48px) do okna, które samo w sobie było już pozycjonowane poniżej paska stanu. Poprzednia konfiguracja przesuwała całe okno o dodatkowe ~48px w dół, tworząc nieestetyczną pustą przestrzeń u góry i powodując, że dolny pasek akcji chował się pod 3-przyciskowym systemowym paskiem nawigacji Androida.
  - **Symetryczne wycentrowanie z czystym marginesem 14dp**: Zastosowano `DialogProperties(usePlatformDefaultWidth = false)` z natywną obsługą ramek okna oraz równomiernym paddingiem `14.dp` z każdej strony. Okno podglądu pliku oraz audytu uprawnień znajduje się teraz idealnie pomiędzy paskiem stanu a paskiem nawigacji, nie kolidując z przyciskami systemowymi.
- **Obsługa dowiązań symbolicznych (symlinków) w eksploratorze plików (`main.rs`, `FileExplorerScreen.kt`, `Models.kt`)**:
  - **Rozpoznawanie symlinków katalogowych po stronie daemona (`main.rs`)**: Poprzednio `WalkDir` sprawdzał jedynie `entry.file_type().is_dir()`, co dla symlinków zwracało `false`, a rozmiar wynosił kilkadziesiąt bajtów (długość ścieżki). Z tego powodu katalogi takie jak `Kod`, `Nuty`, `Projekty`, `Spitfire` czy `UWM` były prezentowane jako zwykłe pliki o rozmiarach 35 B / 42 B. Po aktualizacji daemon sprawdza `entry.file_type().is_symlink()` i podąża za celem za pomocą `std::fs::metadata()`, poprawnie raportując `is_dir = true`, `type = "dir"`, `is_symlink = true` oraz `symlink_target`.
  - **Nawigacja do katalogów-symlinków i wskaźnik `[↗ symlink]`**: W aplikacji mobilnej symlinki prowadzące do katalogów wyświetlają się jako foldery, ich kliknięcie natychmiast przenosi użytkownika do zawartości katalogu (zamiast błędnego otwierania podglądu pliku), a obok nazwy wyświetla się etykieta `[↗ symlink]` wraz ze ścieżką docelową.
- **Wyeliminowanie zamarzania wskaźnika ładowania i wyświetlania „krzaczków” (`FileViewerDialog.kt`, `MeshRepository.kt`, `main.rs`)**:
  - **Wielobajtowa detekcja plików binarnych w daemonie (`is_binary_content`)**: Zastąpiono naiwne sprawdzanie pojedynczego bajtu `0x00` w pierwszych 1024 bajtach kompleksową weryfikacją rozszerzeń binarnych (archiwa, biblioteki, obrazy, bazy danych, dokumenty) oraz analizą pierwszych 8 KB bufora pod kątem bajtów zerowych, znaków kontrolnych i poprawności UTF-8. Pliki binarne nie przesyłają już 500 KB zniekształconych znaków zastępczych UTF-8 (`\uFFFD`).
  - **Zabezpieczenie wątku głównego (UI) w Compose (`FileViewerDialog.kt`)**: W przypadku otwarcia pliku binarnego lub minifikowanego z pojedynczymi liniami o długości dziesiątek tysięcy znaków silnik formatowania tekstu (HarfBuzz/Minikin) blokował wątek UI na kilka sekund, powodując zamarznięcie animacji kręciołka ładowania. Wdrożono automatyczne wykrywanie treści binarnej (`isLikelyBinary`) i przekierowanie do `GenericBinaryCard`, a dla plików tekstowych wprowadzono limit długości pojedynczej linii (2000 znaków), eliminując jakiekolwiek opóźnienia i zamrożenia interfejsu.
  - **Naprawa ucinania etykiet przycisków błędów**: Dodano `softWrap = false` do przycisku „Spróbuj ponownie” (co zapobiega ucinaniu drugiego słowa przez Compose) oraz usunięto zduplikowany przycisk „Zapytaj” na rzecz przycisku w dolnym pasku akcji i opcji bezpośredniego otwarcia w eksploratorze.
